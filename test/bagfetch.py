...
class FetchTest(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        if os.path.exists(
            os.path.join(os.getcwd(), "test", "testbag", "data", "stealin_mah_bag.jpg")
        ):
            os.remove(
                os.path.join(
                    os.getcwd(), "test", "testbag", "data", "stealin_mah_bag.jpg"
                )
            )
        if os.path.exists(
            os.path.join(os.getcwd(), "test", "testbag", "data", "bagitspec.pdf")
        ):
            os.remove(
                os.path.join(os.getcwd(), "test", "testbag", "data", "bagitspec.pdf")
            )
    ...
    def set_fetch_contents(self):
        self.bag.add_fetch_entries(
            [
                {
                    "url": "http://icanhascheezburger.files.wordpress.com/2007/06/stealing_my_bag.jpg",
